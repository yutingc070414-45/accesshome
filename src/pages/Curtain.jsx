...
export default function Curtain() {
  ...
  useEffect(() => {
    fetch(`${API}/api/curtain`).then(r => r.json()).then(rooms => {
      setOpenPct(prev => {
        const next = { ...prev };
        rooms.forEach(room => {
      ...
      <main className="page-wrap">
        ...
        <div className="curtain-grid">
          {ROOMS.map((room, i) => {
            ...
            return (
              <div key={room.key} className={`curtain-card fade-up fade-up-d${i + 1}`}>
                ...
                <div className="curtain-visual">
                  ...
                  <div className="curtain-l" style={{ width: `${closed}%` }}></div>
                  <div className="curtain-r" style={{ width: `${closed}%` }}></div>
                  <div className="curtain-pct">{open}% open</div>
                </div>
                <input type="range" min="0" max="100" value={open}
                  onChange={e => setVal(room.key, parseInt(e.target.value))} />
                <div className="curtain-btns">
                  <button className={`curtain-btn ${open === 0 ? 'primary' : ''}`} onClick={() => setVal(room.key, 0)}>{t('curtain.close')}</button>
                  <button className={`curtain-btn ${open === 100 ? 'primary' : ''}`} onClick={() => setVal(room.key, 100)}>{t('curtain.open')}</button>
                  <button className={`curtain-btn ${open === 50 ? 'primary' : ''}`} onClick={() => setVal(room.key, 50)}>{t('curtain.half')}</button>
                </div>
              </div>
            );
          })}
